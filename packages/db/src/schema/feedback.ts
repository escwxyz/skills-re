...
import type { FeedbackId, SkillId, UserId } from "../utils";
...
export const feedbackTable = sqliteTable(
  "feedback",
  {
    ...
    type: text("type", {
      enum: ["bug", "request", "general", "skill_issue", "skill_display", "skill_takedown"],
    })
      .notNull()
      .default("general"),
  ...
  (table) => [
    index("feedback_skill_id_idx").on(table.skillId),
    index("feedback_skill_slug_idx").on(table.skillSlug),
