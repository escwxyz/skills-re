...
export const appRouter = {
  ...
  changelogs: {
    getById: adminProcedure.changelogs.getById.handler(({ input }) => getChangelogById(input.id)),
    list: publicProcedure.changelogs.list.handler(({ input }) =>
      listPublishedChangelogs(input as { limit?: number } | undefined),
    ),
    listAdmin: adminProcedure.changelogs.listAdmin.handler(({ input }) =>
      listAllChangelogs(input as { limit?: number } | undefined),
    ),
    remove: adminProcedure.changelogs.remove.handler(({ input }) => removeChangelog(input.id)),
    upsert: adminProcedure.changelogs.upsert.handler(({ input }) =>
      upsertChangelog({
