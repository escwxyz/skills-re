...
export const EmailOtpForm = ({ onBack }: { onBack: () => void }) => {
  ...
  const formSchema = z.object({
    email: z.email(m.email_otp_form_please_enter_a_valid_email()),
    otp: z.string(),
  });
  ...
  const form = useAppForm({
    ...
    onSubmit: async ({ value }) => {
      if (!didSend) {
        setErrorMessage(m.email_otp_form_send_a_verification_code_first());
        return;
      }

      if (!value.otp || value.otp.length < 6) {
        setErrorMessage(m.email_otp_form_enter_the_6_digit_code());
        return;
      }
      ...
      try {
        ...
        setErrorMessage(m.email_otp_form_invalid_or_expired_code());
      } finally {
  ...
  const handleSendOtp = async () => {
    ...
    if (!(emailValue && z.email().safeParse(emailValue).success)) {
      setErrorMessage(m.email_otp_form_please_enter_a_valid_email());
      return;
    }
    ...
    try {
      ...
      setDidSend(true);
      setSecondsLeft(300);
      setStatusMessage(m.email_otp_form_verification_code_sent());
    } catch {
      setErrorMessage(m.email_otp_form_could_not_send_code());
    } finally {
      setIsSending(false);
    }
  };
        ...
        <form.AppField name="email">
          {(field) => (
            <Field className="space-y-2">
              <FieldLabel>{m.email_otp_form_email_address()}</FieldLabel>
              <Input
                autoComplete="email"
                className="h-10 text-sm"
                onChange={(event) => field.handleChange(event.target.value)}
                placeholder={m.email_otp_form_email_placeholder()}
                type="email"
            ...
            {(field) => (
              <Field className="space-y-2">
                <div className="flex items-center justify-between">
                  <FieldLabel>{m.email_otp_form_verification_code()}</FieldLabel>
        ...
        <div className="space-y-3">
          <form.Subscribe selector={(state) => state.values.email}>
            {(emailValue) => (
              ...
              >
                {isSending
                  ? m.email_otp_form_sending()
                  : didSend
                    ? m.email_otp_form_resend_verification_code()
                    : m.email_otp_form_send_verification_code()}
              </Button>
            )}
          </form.Subscribe>
            ...
            {([otpValue, canSubmit, isSubmitting]) => (
              ...
              >
                {isVerifying || isSubmitting
                  ? m.email_otp_form_verifying()
                  : m.email_otp_form_verify_and_continue()}
              </Button>
            )}
          </form.Subscribe>
          ...
          <div className="text-muted-foreground flex items-center justify-between text-xs">
            <button
              ...
              type="button"
            >
              {m.email_otp_form_back_to_sign_in_options()}
            </button>
            {didSend && (
              <button
