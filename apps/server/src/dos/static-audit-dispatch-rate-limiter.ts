...
type StaticAuditDispatchRateLimitReservationRequest = AiWorkflowRateLimitReservationRequest;

export type StaticAuditDispatchRateLimitReservation = AiWorkflowRateLimitReservation;
...
export class StaticAuditDispatchRateLimiter extends DurableObject<Env> {
  async fetch(request: Request): Promise<Response> {
    ...
    const input = (await request
      .json()
      .catch(() => ({}))) as StaticAuditDispatchRateLimitReservationRequest;
    const result = await this.reserve(input);
    return Response.json(result);
  }

  private async reserve(
    input: StaticAuditDispatchRateLimitReservationRequest,
  ): Promise<StaticAuditDispatchRateLimitReservation> {
    const dailyLimit = parsePositiveInteger(input.dailyLimit, 1000);
