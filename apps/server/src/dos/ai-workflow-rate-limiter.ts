...
export interface AiWorkflowRateLimitReservationRequest {
  dailyLimit?: number;
  spacingMs?: number;
  units?: number;
}

export interface AiWorkflowRateLimitReservation {
  delaySeconds: number;
  notBeforeMs: number;
}
...
export class AiWorkflowRateLimiter extends DurableObject<Env> {
  async fetch(request: Request): Promise<Response> {
    if (request.method !== "POST") {
      return new Response("Method Not Allowed", { status: 405 });
    }

    const input = (await request.json().catch(() => ({}))) as AiWorkflowRateLimitReservationRequest;
    const result = await this.reserve(input);
    return Response.json(result);
  }

  private async reserve(
    input: AiWorkflowRateLimitReservationRequest,
  ): Promise<AiWorkflowRateLimitReservation> {
